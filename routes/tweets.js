...
router.put('/like', (req, res) => {
  ...
  User.findOne({ token: req.body.token }).then(user => {
    ...
    Tweet.findById(req.body.tweetId).then(tweet => {
      ...
      if (tweet.likes.includes(user._id)) {  // L'utilisateur a déjà aimé le tweet
        Tweet.updateOne({ _id: tweet._id }, { $pull: { likes: user._id } }) // Supprimer l'ID utilisateur au likes
          .then(() => {
            res.json({ result: true });
          });
      } else { // L'utilisateur n'a pas aimé le tweet
        Tweet.updateOne({ _id: tweet._id }, { $push: { likes: user._id } }) // Ajouter l'ID utilisateur au likes
          .then(() => {
            res.json({ result: true });
          });
      }
    ...
    Tweet.findById(req.body.tweetId)
      ...
      .then(tweet => {
        if (!tweet) {
          res.json({ result: false, error: 'Tweet not found' });
          return;
        } else if (String(tweet.author._id) !== String(user._id)) { // ObjectId doit être converti en chaîne pour être comparé
